...
public:
    ...
    DebugEditor(std::shared_ptr<Scene> scenePtr, std::shared_ptr<Rasterizer> rasterizerPtr) : Editor("Debug"), _scenePtr(scenePtr), _rasterizerPtr(rasterizerPtr) {}


    void renderUI() override {
        ImGui::Checkbox("Show Lights", &_rasterizerPtr->debugLights());

        ImGui::RadioButton("Median Split", &BVH::BUILD_TYPE, 0); ImGui::SameLine();
        ImGui::RadioButton("Surface Area Heuristic", &BVH::BUILD_TYPE, 1);
        if (BVH::BUILD_TYPE == 1) {
            ImGui::SliderInt("Split Candidates", &BVH::NUM_SPLIT_CANDIDATES, 1, 20);
        }
        ...
        if (_rasterizerPtr->debugBVH()) {
            ...
            for (size_t i = 0; i < _scenePtr->numOfModels(); i++) {
                maxDepth = std::max(maxDepth, _scenePtr->model(i)->mesh()->bvh()->depth());
            }
            ImGui::SliderInt("BVH depth", &_rasterizerPtr->BVH_debug_depth(), 0, maxDepth);
