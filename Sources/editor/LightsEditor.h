...
public:
    ...
    LightsEditor(std::shared_ptr<Scene> scenePtr, glm::vec3 center, float meshScale) : Editor("Lights"), _scenePtr(scenePtr), _center(center), _meshScale(meshScale) {}
    ...
    void renderUI() override {
        ...
        for (int i = 0; i < _scenePtr->numOfLights(); i++) {
            ...
            if (ImGui::CollapsingHeader(std::string("Light " + std::to_string(i)).c_str())) {
                    ...
                    for (int n = 0; n < IM_ARRAYSIZE(items); n++) {
                        ...
                        if (ImGui::Selectable(items[n], is_selected)) {
                            if (n != light.getType()) {
                                ...
                                switch (n) {
                                case 0:
                                    new_light = std::make_shared<DirectionalLight>(light.color(), light.baseIntensity(), glm::vec3(0.0, -1.0, 0.0));
                                    break;
                                case 1:
                                    new_light = std::make_shared<PointLight>(light.color(), light.baseIntensity(), glm::vec3(0.0), 1.0, 0.0, 0.0);
                                }
                ...
                if (light.getType() == 0) { // Directional light
                    std::shared_ptr<DirectionalLight> dirLight = std::dynamic_pointer_cast<DirectionalLight>(_scenePtr->light(i));
                    ...
                    std::shared_ptr<PointLight> pointLight = std::dynamic_pointer_cast<PointLight>(_scenePtr->light(i));
                    ...
                    ImGui::SliderFloat(("Attenuation constant##" + std::to_string(i)).c_str(), &pointLight->attenuationConstant(), 0.0f, 1.0f);
                    ImGui::SliderFloat(("Attenuation linear##" + std::to_string(i)).c_str(), &pointLight->attenuationLinear(), 0.0f, 1.0f);
                    ImGui::SliderFloat(("Attenuation quadratic##" + std::to_string(i)).c_str(), &pointLight->attenuationQuadratic(), 0.0f, 1.0f);
                }
                ImGui::Indent(-10.0f);
            }
        }
        if (_scenePtr->numOfLights() < MAX_LIGHTS) {
            if (ImGui::Button("Add light")) {
                _scenePtr->add(std::make_shared<PointLight>(glm::vec3(1.0f), 1.0f, glm::vec3(0.0f), 1.0f, 0.0f, 0.0f));
            }
            ImGui::SameLine();
        }
