...
public:
    ...
    inline glm::mat4 computeTransformMatrixCam() const {
        ...
        glm::mat4 rsm = glm::rotate(sm, getRotation()[0], glm::vec3(1.0, 0.0, 0.0));
    ...
    inline glm::mat4 computeViewMatrix() {
        view_mat = inverse(glm::mat4_cast(curQuat) * computeTransformMatrixCam());
        inv_view_mat = glm::inverse(view_mat);
        return view_mat;
    }

    inline glm::mat4 computeProjectionMatrix() {
        proj_mat = glm::perspective(glm::radians(m_fov), m_aspectRatio, m_near, m_far);
