...
RayTracer::RayTracer() :
    m_imagePtr(std::make_shared<Image>(0, 0)) {
}
...
// Unaccelerated raytracing -> slow (and now uncompatible with the model transforms)
Hit RayTracer::traceRay(const Ray& ray, const std::shared_ptr<Scene> scenePtr) {
    ...
    for (size_t j = 0; j < scenePtr->numOfModels(); j++) {
        ...
        Hit tempHit{};
        if (!AABBIntersection(ray, *mesh.bvh()->getRoot()->aabb, tempHit)) continue;
...
Hit RayTracer::traceRayBVH(const Ray& ray, const std::shared_ptr<Scene> scenePtr) {
    Hit hit{};
...
void RayTracer::render(const std::shared_ptr<Scene> scenePtr) {
    ...
    std::cout << "Start ray tracing at " << width << "x" << height << " resolution..." << std::endl;
...
#pragma omp parallel for // Magic
    for (size_t i = 0; i < m_imagePtr->width() * m_imagePtr->height(); i++) {
        int x = i % width;
        int y = i / width;
        Ray ray = scenePtr->camera()->rayAt(glm::vec2(x, y), glm::vec2(width, height));
        ...
        if (hit.hit) {
            ...
            hit.position = glm::vec3(modelMatrix * glm::vec4(hit.position, 1.0));
            normal = glm::normalize(glm::vec3(glm::transpose(inv_modelMatrix) * glm::vec4(normal, 0.0)));
            ...
            for (size_t i = 0; i < scenePtr->numOfLights(); i++) {
                ...
                if (contributes)
                    colorResponse += evaluateRadiance(material, scenePtr->light(i), normalize(normal), wo, hit.position);
            }

            // Reflection
            if (scenePtr->imageParameters().raytracedReflections && material.roughness() < 1.0f) {
                ...
                if (reflectedHit.hit) {
                    Mesh& reflectedMesh = *scenePtr->model(reflectedHit.meshIndex)->mesh();

                    glm::uvec3 reflected_triangle = reflectedMesh.bvh()->triangles[reflectedHit.triangleIndex];
                    const glm::vec3& reflected_a = reflectedMesh.vertexPositions()[reflected_triangle.x];
                    const glm::vec3& reflected_b = reflectedMesh.vertexPositions()[reflected_triangle.y];
                    const glm::vec3& reflected_c = reflectedMesh.vertexPositions()[reflected_triangle.z];

                    glm::vec3 reflected_barycentric = getBarycentric(reflectedHit.position, { reflected_a, reflected_b, reflected_c });
                    // Normal
                    glm::vec3 reflected_normal =
                        reflected_barycentric.x * reflectedMesh.vertexNormals()[reflected_triangle.x] +
                        reflected_barycentric.y * reflectedMesh.vertexNormals()[reflected_triangle.y] +
                        reflected_barycentric.z * reflectedMesh.vertexNormals()[reflected_triangle.z];
                    reflected_normal = glm::normalize(reflected_normal);

                    // // BRDF
                    Material reflectedMaterial = scenePtr->model(reflectedHit.meshIndex)->material();

                    glm::vec3 reflectedColor(0.0f);
                    glm::vec3 reflectedWo = glm::normalize(hit.position - reflectedHit.position);

                    for (size_t i = 0; i < scenePtr->numOfLights(); i++) {
                        std::shared_ptr<AbstractLight> light = scenePtr->light(i);
                        reflectedColor += evaluateRadiance(reflectedMaterial, scenePtr->light(i), normalize(reflected_normal), reflectedWo, reflectedHit.position);
                    }

                    colorResponse += reflectedColor * BRDF(material).F(reflectedRay.direction(), -ray.direction(), normal);
                }
                else {
                    glm::vec3 reflectedColor = scenePtr->backgroundColor();
                    colorResponse += BRDF(material).F(reflectedRay.direction(), -ray.direction(), normal) * reflectedColor;
                }
            }
