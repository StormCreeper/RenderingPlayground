...
void GPU_Raytracer::init(const std::string& basePath, const std::shared_ptr<Scene> scenePtr) {
    initScreenQuad();
...
void GPU_Raytracer::loadShaderProgram(const std::string& basePath) {
    ...
    try {
        ...
        m_raytracingShaderProgramPtr = ShaderProgram::genBasicShaderProgram(shaderPath + "/RaytracingVertexShader.glsl",
...
void GPU_Raytracer::render(std::shared_ptr<Scene> scenePtr) {
    ...
    m_raytracingShaderProgramPtr->set("inv_view_mat", glm::inverse(scenePtr->camera()->computeViewMatrix()));
    m_raytracingShaderProgramPtr->set("inv_proj_mat", glm::inverse(scenePtr->camera()->computeProjectionMatrix()));
    m_raytracingShaderProgramPtr->set("proj_mat", scenePtr->camera()->computeProjectionMatrix());
    m_raytracingShaderProgramPtr->set("view_mat", scenePtr->camera()->computeViewMatrix());
    ...
    scenePtr->imageParameters().setUniforms(*m_raytracingShaderProgramPtr, "imageParameters");

    size_t numOfLights = scenePtr->numOfLights();
    m_raytracingShaderProgramPtr->set("numOfLights", static_cast<int> (numOfLights));
    for (size_t i = 0; i < numOfLights; i++) {
        scenePtr->light(i)->setUniforms(*m_raytracingShaderProgramPtr, "lights[" + std::to_string(i) + "]");
    }
    ...
    for (int i = 0; i < numOfTextures; i++) {
        ...
        m_raytracingShaderProgramPtr->set("textures[" + std::to_string(i) + "]", i);
    }
...
void buildGPUData(
    ...
    for (size_t i = 0; i < scenePtr->numOfModels(); i++) {
        ...
        for (size_t j = 0; j < nodes.size(); j++) {
            ...
            node.triangle_count = nodes[j]->childIndex == 0 ? nodes[j]->num_triangles : 0;
            node.offset = nodes[j]->childIndex == 0 ? nodes[j]->first : nodes[j]->childIndex;
...
void GPU_Raytracer::createSSBOs(std::shared_ptr<Scene> scenePtr) {
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(SSBO_Vertex) * vertices.size(), vertices.data(), GL_DYNAMIC_COPY);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    glGenBuffers(1, &m_trianglesSSBO);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_trianglesSSBO);
    glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(glm::uvec4) * triangles.size(), triangles.data(), GL_DYNAMIC_COPY);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    glGenBuffers(1, &m_modelsSSBO);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_modelsSSBO);
    glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(SSBOModel) * models.size(), models.data(), GL_DYNAMIC_COPY);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    glGenBuffers(1, &m_bvhSSBO);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_bvhSSBO);
    glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(SSBO_BVH_Node) * bvh_nodes.size(), bvh_nodes.data(), GL_DYNAMIC_COPY);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
}
...
void GPU_Raytracer::updateSSBOs(std::shared_ptr<Scene> scenePtr) {
    ...
    // glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_verticesSSBO);
    // glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(SSBO_Vertex) * vertices.size(), vertices.data());
    // glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    // glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_trianglesSSBO);
    // glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(glm::uvec4) * triangles.size(), triangles.data());
    // glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_modelsSSBO);
    glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(SSBOModel) * models.size(), models.data());
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    // glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_bvhSSBO);
    // glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(SSBO_BVH_Node) * bvh_nodes.size(), bvh_nodes.data());
    // glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
}

GLuint GPU_Raytracer::genGPUBuffer(size_t elementSize, size_t numElements, const void* data) {
    GLuint vbo;
...
GLuint GPU_Raytracer::genGPUVertexArray(GLuint posVbo, GLuint ibo, bool hasNormals, GLuint normalVbo) {
    GLuint vao;
