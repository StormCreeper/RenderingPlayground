...
void Rasterizer::loadShaderProgram(const std::string& basePath) {
    ...
    try {
        std::string shaderPath = basePath + "/" + SHADER_PATH;
        m_debugShaderProgramPtr = ShaderProgram::genBasicShaderProgram(shaderPath + "/DebugVertexShader.glsl",
...
void Rasterizer::render(std::shared_ptr<Scene> scenePtr) {
    ...
    for (size_t i = 0; i < numOfLights; i++) {
        scenePtr->light(i)->setUniforms(*m_pbrShaderProgramPtr, "lights[" + std::to_string(i) + "]");
    }

    scenePtr->imageParameters().setUniforms(*m_pbrShaderProgramPtr, "imageParameters");
...
void Rasterizer::renderDebug(std::shared_ptr<Scene> scenePtr) {
    ...
    m_debugShaderProgramPtr->set("projectionMat", scenePtr->camera()->computeProjectionMatrix());
    ...
    if (m_debugBVH) {
        ...
        for (size_t i = 0; i < numOfModels; i++) {
            std::vector<AABB> aabbs = scenePtr->model(i)->getAABBs(m_BVH_debug_depth);
            glm::mat4 modelMatrix = scenePtr->model(i)->mesh()->getTransformMatrix();
            m_debugShaderProgramPtr->set("modelViewMat", viewMatrix * modelMatrix);

            for (size_t j = 0; j < aabbs.size(); j++) {
                m_debugShaderProgramPtr->set("begin_corner", aabbs[j].begin_corner);
...
GLuint Rasterizer::genGPUVertexArray(GLuint posVbo, GLuint ibo, bool hasNormals, GLuint normalVbo, bool hasUVs, GLuint uvVBO, bool tangentSpace, GLuint tangentVbo, GLuint bitangentVbo) {
    GLuint vao;
...
GLuint Rasterizer::toGPU(std::shared_ptr<Mesh> meshPtr) {
    GLuint posVbo = genGPUBuffer(3 * sizeof(float), meshPtr->vertexPositions().size(), meshPtr->vertexPositions().data());
    GLuint normalVbo = genGPUBuffer(3 * sizeof(float), meshPtr->vertexNormals().size(), meshPtr->vertexNormals().data());
    GLuint uvVbo = genGPUBuffer(2 * sizeof(float), meshPtr->vertexUVs().size(), meshPtr->vertexUVs().data());
    GLuint tangentVbo = genGPUBuffer(3 * sizeof(float), meshPtr->vertexTangents().size(), meshPtr->vertexTangents().data());
    GLuint bitangentVbo = genGPUBuffer(3 * sizeof(float), meshPtr->vertexBitangents().size(), meshPtr->vertexBitangents().data());
    GLuint ibo = genGPUBuffer(sizeof(glm::uvec3), meshPtr->triangleIndices().size(), meshPtr->triangleIndices().data());
    GLuint vao = genGPUVertexArray(posVbo, ibo, true, normalVbo, true, uvVbo, true, tangentVbo, bitangentVbo);
    return vao;
}
...
void Rasterizer::initDebugCube() {
    ...
    m_debugCubeVao = genGPUVertexArray(
        ...
        genGPUBuffer(3 * sizeof(unsigned int), 12, iData.data()),
        false, 0,
        false, 0,
        false, 0, 0
    );
