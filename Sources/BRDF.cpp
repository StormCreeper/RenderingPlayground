...
float BRDF::NormalDistributionGGX(glm::vec3 n, glm::vec3 wh, float alpha) {
	float nom = sqr(alpha);
	float denom =
		glm::pi<float>() * sqr(1 + (sqr(alpha) - 1.0f) * sqr(glm::dot(n, wh)));
...
glm::vec3 BRDF::Fresnel(glm::vec3 wi, glm::vec3 wh, glm::vec3 F0) {
	return F0 + (glm::vec3(1.0f) - F0) *
					pow3(glm::vec3(1.0f) -
							 glm::max(glm::vec3(0.0f), glm::dot(wi, wh)),
						 5);
}
...
glm::vec3 BRDF::F(const glm::vec3& wi, const glm::vec3& wo,
				  const glm::vec3& n) {
	...
	return (fd * (1.0f - material.metalness()) + fs) *
		   glm::max(glm::dot(wi, n), 0.0f);
}
...
glm::vec3 evaluateTotalRadiance(const Scene& scene, const glm::vec3& frag_pos,
	...
	for (int i = 0; i < scene.numOfLights(); i++) {
		colorResponse += evaluateRadiance(material, scene.light(i),
										  normalize(fNormal), wo, frag_pos);
	}
