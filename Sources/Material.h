...
public:
    ...
    Material(const glm::vec3& albedo, float roughness, float metalness, const glm::vec3& F0, bool transparent = false, float base_reflectance = 0.04f, float ior = 1.5f, float absorption = 0.0f)
        : _albedo(albedo), _roughness(roughness), _metalness(metalness), _F0(F0),
        _transparent(transparent), _base_reflectance(base_reflectance), _ior(ior), _absorption(absorption),
        _albedoTex(-1), _roughnessTex(-1), _aoTex(-1), _metalnessTex(-1),
        _normalTex(-1), _heightTex(-1), _heightMult(0.0f) {
    }
