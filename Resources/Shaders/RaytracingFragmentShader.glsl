...
void fresnelDielectric(in float ct1, in float n1, in float n2, out vec2 R, out vec2 phi) {
...
void fresnelConductor(in float ct1, in float n1, in float n2, in float k, out vec2 R, out vec2 phi) {
...
vec3 evaluateRadianceDirectional(Material mat, LightSource source, vec3 n, vec3 pos, Ray ray, vec2 uv) {
    mat.albedo = sampleTex(uv, mat.albedoTex, vec4(mat.albedo, 1.0)).rgb;
    ...
    mat.roughness = sampleTex(uv, mat.roughnessTex, vec4(mat.roughness)).x;

    vec3 fs = F_Specular(n, wi, wo, mat) * source.color * source.intensity;

    mat.metalness = sampleTex(uv, mat.metalnessTex, vec4(mat.metalness)).x;
...
vec3 shade(vec3 normal, Hit hit, Material material, Ray ray, vec2 uv, bool shadows = false) {
    ...
    if(hit.position.y < -0.5) {
        ...
        float radius = 0.3;
        float dist = length(hit.position.xz - center);
        float fact = exp(-dist / radius) * (1. + 0.5 * sin(hit.position.x * 10.) * sin(hit.position.z * 15.));
        material.Dinc *= fact;
