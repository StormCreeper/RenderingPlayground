...
glm::vec3 LessThan(glm::vec3 f, float value) {
	return glm::vec3(f.x < value ? 1.0f : 0.0f, f.y < value ? 1.0f : 0.0f,
					 f.z < value ? 1.0f : 0.0f);
}
...
glm::vec3 LinearToSRGB(glm::vec3 rgb) {
	...
	return mix(glm::pow(rgb, glm::vec3(1.0f / 2.4f)) * 1.055f - 0.055f,
			   rgb * 12.92f, LessThan(rgb, 0.0031308f));
}

glm::vec3 SRGBToLinear(glm::vec3 rgb) {
	rgb = glm::clamp(rgb, 0.0f, 1.0f);

	return mix(glm::pow(((rgb + 0.055f) / 1.055f), glm::vec3(2.4f)),
			   rgb / 12.92f, LessThan(rgb, 0.04045f));
}
